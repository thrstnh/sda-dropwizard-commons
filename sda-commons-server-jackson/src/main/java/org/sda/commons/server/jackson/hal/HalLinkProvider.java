...
public class HalLinkProvider implements Feature {
  ...
  /**
   * Creates a {@linkplain HALLink} based on JAX-RS annotated parameters in the invoked method using
   * {@linkplain HalLinkInvocationStateUtility#methodOn(Class)}
   *
   * <p>Example:
   *
   * <pre>
   ...
   * @throws InvalidHalLinkInvocationException - If no method invocation is provided via {@linkplain
  ...
  public HALLink linkTo(Object invocation) {
    ...
    if (invocation != null || !methodInvocationState.isProcessed()) {
      throw new InvalidHalLinkInvocationException("No proxied method invocation processed.");
    }
